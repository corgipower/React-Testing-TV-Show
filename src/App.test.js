...
test('renders from api', async () => {
  ...
  userEvent.click(selectSeasonButton);
  render(<Dropdown />)
  const clickSeasonOne = await findByText(/Season 1/i);
  expect('Season 1').toBeVisible;
  userEvent.click(clickSeasonOne);
  expect('Chapter Eight: The Upside Down').toBeVisible;
  expect(/Chapter Nine: The Gate jklielaksd/i).toBeVisible; //this should fail, but doesn't
});
