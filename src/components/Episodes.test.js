...
test('renders from api', async () => {
  ...
  const {findByText, getByText} = render(<Episodes episodes={data} />);
  const chapterEight = findByText(/Chapter Eight: The Upside Down/i);
  const chapterNine = findByText(/Chapter Nine: The Gate/i);//passes
  await waitFor(() => expect(findByText('Chapter Nine: The Gate')).toBeInTheDocument); //passes
  await waitFor(() => expect(findByText('Chapter Nine: The Gate')).not.toBeInTheDocument); //passes
  expect(findByText('Chapter Nine: The Gate')).not.toBeInTheDocument; //passes
  expect(findByText('Chapter Nine: The Gate')).toBeInTheDocument; //passes
  await waitFor(() => expect(findByText('Chapter Nine: The Gate'))); //passes
});
